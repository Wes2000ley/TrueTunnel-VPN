...
// Static buffers for logging and message input
			...
			const size_t max_log_lines = 5000;
                        ...
                        if (ImGui::Button("Connect")) {
                                ...
                                auto append_line = [&](const std::string &message,
                                ...
                                g_vpn_daemon->set_event_callback([&log_lines, &log_mutex, max_log_lines, format_with_timestamp](const VpnDaemon::TelemetryEvent &event) {
                                        std::string payload;
                                        switch (event.type) {
                                ...
                                } else if (!g_vpn_daemon->start(config)) {
                                        append_line("[!] Failed to start VPN daemon");
                                }
                        }
			ImGui::SameLine();
