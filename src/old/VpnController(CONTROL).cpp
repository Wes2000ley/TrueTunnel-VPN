...
// use `adapter.get()` as needed (e.g., WintunStartSession)
		if (is_server) {
			...
			gateway = "10.10.100.1"; // Not needed — it's the gateway

			char request_buf[64] = {};
			int bytes = SSL_read(ssl_.get(), request_buf, sizeof(request_buf) - 1);
			request_buf[bytes] = '\0';

			if (strcmp(request_buf, "VPN_REQUEST_CONFIG") != 0) {
				std::cerr << "[!] Unexpected message: " << request_buf << "\n";
				cleanup_ssl_and_socket();
				return;
			}
			...
			std::cout << "[*] Assigned IP to client: " << *assigned_ip << "\n";
		} else {
			const char *request = "VPN_REQUEST_CONFIG";
			SSL_write(ssl_.get(), request, static_cast<int>(strlen(request)));

			char cfg_buf[256] = {};
			int n = SSL_read(ssl_.get(), cfg_buf, sizeof(cfg_buf) - 1);
			if (n <= 0) {
				std::cerr << "[!] Failed to receive VPN config from server\n";
				cleanup_ssl_and_socket();
				return;
			}
			cfg_buf[n] = 0;
			std::string cfg(cfg_buf);

			std::smatch match;
			std::regex r("IP=(.*?);GW=(.*?);MASK=(.*?)(;|$)");
			if (!std::regex_search(cfg, match, r)) {
				std::cerr << "[!] Invalid VPN config format: " << cfg << "\n";
				cleanup_ssl_and_socket();
				return;
			}
			...
			std::cout << "[*] Received IP config from server:\n";
			std::cout << "    IP     = " << local_ip << "\n";
			std::cout << "    GW     = " << gateway << "\n";
			std::cout << "    MASK   = " << subnetmask << "\n";
		}
